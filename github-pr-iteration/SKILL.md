...
5. If that check finds an open PR for the issue, stop the new-PR path immediately and continue on that existing PR/branch only when that PR is yours. If the matching open PR belongs to another contributor or fork, do not hijack it and do not create a duplicate; surface the conflict to the user instead.
...
# PR creation + checks
 ...
 gh pr list --repo <owner/repo> --state open --limit 100 --search '"[<issue-number>]" in:title' --json number,url,state,title,headRefName,isCrossRepository,headRepositoryOwner,maintainerCanModify
